...
# with mail.bioco.ch this seems to work better:
...
def send_mail(subject, message, from_email, to_emails):
    ...
    if settings.DEBUG is False:
        ...
        print "Mail intended for " + ", ".join(to_emails).encode(
            'utf-8') + " rerouted to " + settings.DEBUG_EMAIL_ADDRESS.encode('utf-8')

    server = smtplib.SMTP(settings.EMAIL_HOST)
    # server.set_debuglevel(1)
    status_login = server.login(settings.EMAIL_HOST_USER, settings.EMAIL_HOST_PASSWORD)

    if len(okmails) > 0:
        for amail in okmails:
            # Original django-only mailer, failing on openshift
            ...
            msg['From'] = from_email
            msg['To'] = amail
            res = server.sendmail(from_email, [amail], msg.as_string())
    ...
    message_verbose = (message + '\n\nCopy of original sent to: ' + ', '.join(to_emails)).encode('utf-8')
    msg_debug = MIMEText(message_verbose, 'plain', 'utf-8')
    msg_debug['Subject'] = subject.encode('utf-8')
    msg_debug['From'] = from_email
    msg_debug['To'] = settings.DEBUG_EMAIL_ADDRESS
    server.sendmail(from_email, [settings.DEBUG_EMAIL_ADDRESS], msg_debug.as_string())
...
def send_mail_multi(email_multi_message):
    ...
    if settings.DEBUG is False:
        ...
        okmails = [settings.DEBUG_EMAIL_ADDRESS]
        print "Multi-Mail intended for " + ", ".join(to_emails).encode(
            'utf-8') + " rerouted to " + settings.DEBUG_EMAIL_ADDRESS.encode('utf-8')
    ...
    res = mail.send_mail(email_multi_message.subject, message_verbose, email_multi_message.from_email,
                         [settings.DEBUG_EMAIL_ADDRESS], fail_silently=False)
...
def send_new_loco_in_taetigkeitsbereich_to_bg(area, loco):
    send_mail('Neues Mitglied im Taetigkeitsbereich ' + area.name,
              'Soeben hat sich ' + loco.first_name + " " + loco.last_name + ' in den Taetigkeitsbereich ' + area.name + ' eingetragen',
              SENDER_EMAIL_ADDRESS, [area.coordinator.email])
